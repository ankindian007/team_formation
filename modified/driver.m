...
for i = 1:size(task_mat,1) 
    ...
    % Label Propagation    
    [M,W_selected,R,S,K,qual_auth_names,idx] = hyperagent_newest(R,K,alpha);
    
    % MM Algorithm Run
    %[S_sub,S,U,x_star] = MM(M,R,S,K,qual_auth_names,idx);
    
    % MM Weighted Algorithm Run
    [S_sub,S,U,x_star] = MMW(M,R,S,K,qual_auth_names,idx,W_selected);
